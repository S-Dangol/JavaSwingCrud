...
public class BooksDaoImpl implements BooksDao {
    ...
    public int save(Books book) throws SQLException, ClassNotFoundException {
        String insertSQL = "insert into books ( name, author, publication, price, id2) values(?,?,?,?,?)";
        ...
        prepareStatement.setDouble(5, book.getId2());
        return prepareStatement.executeUpdate();
    }

    @Override
    public int update(Books book, int id) throws SQLException, ClassNotFoundException {
        String updateSQL = "update books set name = ?, author = ?, publication = ?, price = ?, id2=? where id = ?";
        ...
        preparedStatement.setDouble(5, book.getId2());
    ...
    public Books findOne(int id) throws SQLException, ClassNotFoundException {
        ...
        while (resultSet.next()) {
            ...
            book.setId2(resultSet.getDouble("id2"));
        }
        return book;
    }
    ...
    public List<Books> search(String query) throws SQLException, ClassNotFoundException {
        List<Books> books = new ArrayList<>();
        PreparedStatement preparedStatement = getConnection().prepareStatement("select *from books where name like concat ('%' ? '%')"
                + "or author like concat ('%' ? '%') or publication like concat ('%' ? '%')"
                + " or cast(id as char) like ('%' ? '%') or cast(price as char) like ('%' ? '%') or cast(id2 as char) like ('%' ? '%')");
